...
import { wordFilterURL } from '../../../Navigation';
...
const WordFilterItem = (props: Props) => {
  const { word } = props.filter;
    ...
    <View>
      <View style={StyleSheet.styles.discoveryCenter.topContainer}>
        <View style={StyleSheet.styles.discoveryCenter.leftContainer}>
          <TouchableOpacity>
            <Text style={[styles.button, {color: '#9B9B9B'}]}>WORD</Text>
          </TouchableOpacity>
        </View>
      </View>
      <View style={StyleSheet.styles.discoveryCenter.bottomContainer}>
        <Text>Matching</Text>
        <TouchableOpacity hitSlop={{top: 10, left: 5, bottom: 10, right: 0}} style={styles.filterButton} onPress={() => props.onPressEditFilter(wordFilterURL({title: Localizable.t('word'), filter: props.filter}))}>
          <Text style={styles.filterButtonTitle}>{word}</Text>
