...
const ChronologyFilterItem = (props: Props) => {
  const { chronologyID } = props.filter;
  ...
  return (
    <View>
      ...
      <View style={StyleSheet.styles.discoveryCenter.bottomContainer}>
        <Text>In</Text>
        <TouchableOpacity hitSlop={{top: 10, left: 5, bottom: 10, right: 0}} style={styles.filterButton} onPress={() => props.onPressEditFilter(chronologyFilterURL({title: Localizable.t('time-period'), filter: props.filter}))}>
...
export default ChronologyFilterItem;
