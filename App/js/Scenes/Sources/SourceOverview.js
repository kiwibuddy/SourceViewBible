...
import { BarChart, SpheresBarChart, WordCloud } from '../../Components/Charts';
...
import { bookURL, sourceHelpURL, occurrencesURL, readerURL, sourceURL, sourceSpheresURL, sourceWordsURL } from '../../Navigation';
...
export default class SourceOverview extends Component {
  ...
  static renderMenuOptions(props: Object) {
    return <MenuOption key="help" text={Localizable.t('help')} onSelect={() => props.navigate(sourceHelpURL({ title: Localizable.t('help'), modal: true }))} />;
  }
    ...
    return (
      <View>
        <TouchableOpacity onPress={this._onPressWords}>
          <WordCloud backgroundColors={this.principalColor.gradient.big} style={styles.wordCloud}>
        ...
        <View style={styles.sourcesCellContainer}>
          <View style={styles.sourcesLeftContainer}>
            <Text style={StyleSheet.styles.cell.subtitle}>
              {Localizable.t('sources.count', { count: sourceCount, localizedCount: Localizable.toNumber(sourceCount, { precision: 0 }) })}
            </Text>
          </View>
  ...
  _renderRow = (row: Object, sectionID: any) => {
        ...
        <View style={[styles.sourcesCellContainer, { paddingVertical: 10 }]}>
          <View style={styles.sourcesLeftContainer}>
            <TouchableOpacity onPress={() => this._onPressActantIcon(actant)}>
              <SourceIcon principalSourceType={principalSourceType} source={actant} style={[styles.sourceAvatar]} size={20} />
            </TouchableOpacity>
  ...
  _renderMetaData = () => {
      ...
      const profession = (
        <View key="profession">
          <View style={styles.listItemContainer}>
            <Icon name="metadata-profession" size={20} style={[styles.listItemIcon, { color: '#59626A', alignSelf: 'auto', paddingTop: 5 }]} />
    ...
    return <View style={styles.listContainer}>{metaData}</View>;
  };
  ...
  _renderMetaValue = (title: string, icon: string, value: string) => {
    return (
      <View key={title}>
        <View style={styles.listItemContainer}>
          <Icon name={icon} size={20} style={[styles.listItemIcon, { color: '#59626A' }]} />
          <View style={styles.listItem}>
            <View style={styles.sourcesCellContainer}>
              ...
              <View style={[styles.sourcesRightContainer, { marginRight: -8 }]}>
                <Text numberOfLines={1} style={[StyleSheet.styles.cell.valuetitlemedium, { textAlign: 'right' }]}>
                  {value}
                </Text>
              </View>
            </View>
  ...
  _onPressWords = () => {
    ...
    this.props.navigate(
      sourceWordsURL({
        ...
        title: source.name,
        description: Localizable.t('source-words', { name: source.name }),
      })
    );
  };
  ...
  _onPressSpheres = () => {
    ...
    this.props.navigate(
      sourceSpheresURL({
        ...
        bookID: book && book.id,
        title: source.name,
        description: Localizable.t('source-spheres', { name: source.name }),
      })
    );
  };
