    ...
    return (
      <View style={styles.container}>
        <NavigationHeader
          ...
          renderLeftComponent={() => (
            <NavigationBarButton title={backTitle} titleStyle={StyleSheet.styles.navigationBar.doneButtonTitle} onPress={this._onPressBack} />
          )}
        />
        <ListView
  ...
  _renderRow = (occurrence: Object, sectionID: any, rowID: any) => {
    ...
    return (
      <TouchableOpacity key={occurrence.id} style={styles.listItemContainer} onPress={() => this._navigate(route)}>
        <Text allowFontScaling={false} style={StyleSheet.styles.cell.occurrence}>
          {number}
        </Text>
