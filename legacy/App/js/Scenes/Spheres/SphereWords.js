...
export default class SphereWords extends Component {
  ...
  _renderHeader = () => {
    ...
    return (
      <WordCloud backgroundColors={Colors.spheres[sphere.id].gradient.big} style={styles.wordCloud}>
  ...
  _onPressWord = (word: Object) => {
    const { sphere } = this.state;
    WordOccurrence.occurrences(word.string, { sphere: sphere.feature }).then(occurrences => {
